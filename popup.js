        ...
        func: (gifUrl) => {
          if (!document.getElementById('ai-mascot')) {
            ...
            mascot.addEventListener('click', () => {
              const userInput = prompt("Ask Hatsune Miku something:");
              const promptAI = `You are a 2D character named Hatsune Miku, Imitate and act out the traits of Hatsune Miku, Just answer briefly but not too briefly. User Input: ${userInput}`;
              if (userInput) {
                ...
                fetch(`https://api-rangestudio.vercel.app/api/gemini?text=${encodeURIComponent(promptAI)}`)
                  .then(res => res.json())
                  .then(data => {
